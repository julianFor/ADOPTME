...
const NavbarFundacion = () => {
  ...
  // RESPONSIVE: estado del menú móvil
  ...
  // Helper para resaltar link activo (sin alterar estilos existentes en desktop)
  const active = (path) =>
    `pb-1 ${
  ...
  return (
    <header className="flex justify-between items-center px-8 py-4 bg-white rounded-full shadow-md max-w-[1200px] mx-auto relative">
      <Link to="/" onClick={() => setOpen(false)}>
        {/* Mantengo el tamaño original que ya tenías en desktop */}
        <img src={logo} alt="AdoptMe logo" className="w-100 h-12" />
      </Link>

      {/* NAV DESKTOP (sin cambios visuales) */}
      <nav className="hidden md:flex gap-6 text-lg font-medium">
        ...
        <Link to="/dashboard/adminFundacion" className="text-gray-700 hover:text-purple-600">Gestión</Link>
      </nav>

      {/* Acciones derecha DESKTOP (igual que original) */}
      ...
      <button
        ...
        aria-expanded={open}
        aria-controls="mobile-menu-fundacion"
      >
        {open ? <FiX /> : <FiMenu />}
      </button>

      {/* PANEL MÓVIL (mismo contenido que nav + acciones) */}
  <div
  id="mobile-menu-fundacion"
...
>
  <div className="flex flex-col gap-3 p-4 text-base font-medium">
    ...
    <Link to="/dashboard/adminFundacion" className="text-gray-700 hover:text-purple-600" onClick={() => setOpen(false)}>Gestión</Link>
