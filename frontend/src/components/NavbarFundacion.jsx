...
const NavbarFundacion = () => {
  ...
  const renderProfileModal = () => {
    ...
    const imagenPorRol = {
      admin: '/GatoAdmin.jpg',
      adminFundacion: '/GatoFundacion.jpeg',
  ...
  return (
    <header className="flex justify-between items-center px-8 py-4 bg-white rounded-full shadow-md max-w-[1200px] mx-auto relative">
      <Link to="/" onClick={() => setOpen(false)}>
        <img src={logo} alt="AdoptMe logo" className="h-12 w-auto" />
      </Link>
      ...
      <nav className="hidden md:flex gap-6 text-lg font-medium">
        ...
        <Link to="/dashboard/adminFundacion" className={active('/dashboard/adminFundacion')}>Gestión</Link>
      </nav>

      <div className="hidden md:flex gap-3 relative">
        <div className="relative">
          <button
            onClick={() => setShowNotificaciones((prev) => !prev)}
          ...
          >
            ...
            {noLeidas > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{noLeidas}</span>
            )}
          </button>
      ...
      <div
        className={`md:hidden absolute left-0 right-0 top-full mx-4 mt-2 rounded-2xl bg-white shadow-xl overflow-hidden transition-all z-[10000] ${
      ...
      >
        <div className="flex flex-col gap-3 p-4 text-base font-medium">
          ...
          <Link to="/dashboard/adminFundacion" className={active('/dashboard/adminFundacion')} onClick={() => setOpen(false)}>Gestión</Link>
          ...
          <div className="pt-2 mt-1 border-t border-gray-100 flex items-center justify-between">
            <div className="flex gap-3">
              ...
              >
                ...
                {noLeidas > 0 && (
                  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center">{noLeidas}</span>
                )}
              </button>
...
export default NavbarFundacion;
