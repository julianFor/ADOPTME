...
function NavbarAuth({ gestionPath }) {
  ...
  const renderProfileModal = () => {
    ...
    const avatar = AVATAR_POR_ROL[user?.role] || '/GatoAdoptante.png';
      ...
      <div className="absolute top-20 right-8 bg-white shadow-xl rounded-xl p-6 w-96 z-50">
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            className="border px-4 py-2 rounded"
          />
          <button className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 transition" onClick={handleUpdate}>
            Guardar cambios
          </button>
          <button className="text-sm text-gray-500 hover:underline text-center" onClick={() => setEditing(false)}>
            Cancelar
          </button>
        </div>
      </div>
  ...
  return (
    <header className="flex justify-between items-center px-8 py-4 bg-white rounded-full shadow-md max-w-[1200px] mx-auto relative">
      ...
      <nav className="hidden md:flex gap-6 text-lg font-medium">
        ...
        <Link to={gestionPath} className={active(gestionPath)}>Gestión</Link>
      </nav>
      ...
      >
        <div className="flex flex-col gap-3 p-4 text-base font-medium">
          ...
          <Link to={gestionPath} className={active(gestionPath)} onClick={() => setOpen(false)}>Gestión</Link>
