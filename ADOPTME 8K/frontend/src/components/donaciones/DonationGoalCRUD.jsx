...
function DonationGoalCRUD() {
  ...
  const fetchGoals = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/donation-goals");
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingId
      ? `http://localhost:3000/api/donation-goals/${editingId}`
      : "http://localhost:3000/api/donation-goals";
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/donation-goals/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-purple-700 mb-6">🎯 Gestión de Metas de Donación</h2>
