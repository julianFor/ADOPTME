...
function DonationGoalCRUD() {
  ...
  const fetchGoals = () => {
    fetch("http://localhost:5000/api/donation-goal")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSubmit = (e) => {
    ...
    const url = editingId
      ? `http://localhost:5000/api/donation-goal/${editingId}`
      : "http://localhost:5000/api/donation-goal";
  ...
  const handleDelete = (id) => {
    ...
    fetch(`http://localhost:5000/api/donation-goal/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then(() => fetchGoals())
